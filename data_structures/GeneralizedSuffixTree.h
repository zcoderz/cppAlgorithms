//
// Created by usman on 9/25/22.
//
//Code taken from : https://www.geeksforgeeks.org/ukkonens-suffix-tree-construction-part-6/?ref=lbp
#ifndef CPPALGORITHMS_GENERALIZEDSUFFIXTREE_H
#define CPPALGORITHMS_GENERALIZEDSUFFIXTREE_H
...
private:
    ...
    void doPrint(Node *n, int labelHeight)
    {
        if(n == NULL)
        {
            return;
        ...
        if(n->suffixIndex == -1) //If it is internal node
        {
            cout << "internal node of label height " << labelHeight << " start index " << n->start << " end index " <<  *n->end << endl;
            for (i = 0; i < MAX_CHAR; i++)
            {
                if(n->children[i] != NULL)
                {
                    doPrint(n->children[i], labelHeight + edgeLength(n->children[i]));
                }
            }
        }
        else if(n->suffixIndex > -1 ) {
            cout << "node address is " << n <<  "node contains " << text.substr(n->suffixIndex, labelHeight - edgeLength(n)) << endl;
        }
    }
...
public:
    ...
    void buildSuffixTree(string strText)
    {
        text = strText + "$";
        size = text.size();
        int i;
...
#endif //CPPALGORITHMS_GENERALIZEDSUFFIXTREE_H
